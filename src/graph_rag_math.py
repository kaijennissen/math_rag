...
def main(recreate_graph: bool = False):
    ...
    if recreate_graph:
        ...
        raw_documents = WikipediaLoader(query="Continuous function").load()
        # Define chunking strategy
        text_splitter = TokenTextSplitter(chunk_size=512, chunk_overlap=24)
        documents = text_splitter.split_documents(raw_documents[:3])

        splitter = SentenceTransformersTokenTextSplitter(
            tokens_per_chunk=64, chunk_overlap=0, model_name="intfloat/e5-base-v2"
        )

        text_token_count = splitter.split_documents(raw_documents[:3])
